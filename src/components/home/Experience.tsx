  ...
  const inner = (
    <div className="flex gap-6">
      ...
      <div className="flex flex-col items-center">
        <span className="bg-background mt-1 size-3 shrink-0 rounded-full border-2 border-[var(--brand-violet)] ring-4 ring-[var(--brand-violet)]/10" />
        {!last && <span className="bg-border mt-1 w-px flex-1" />}
      </div>

      {/* Card content */}
      <div className={cn("pb-10", last && "pb-0")}>
        <p className="font-mono text-xs tracking-widest text-[var(--brand-cyan)] uppercase">
          {exp.period}
        </p>
          ...
          <ul className="mt-3 space-y-1.5">
            {exp.description.map((d, i) => (
              <li key={i} className="text-muted flex gap-2 text-sm">
                <span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-[image:var(--gradient-brand)]" />
                <span>{d}</span>
              </li>
            ))}
          </ul>
